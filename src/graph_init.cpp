...
void ocl_init(const char* progName, const char* kernelNameEntryDiscover, const char* kernelNameComputeMetrics);
...
//compute_metrics_all_in_host

int *queue, *next_queue;
cl_event* compute_metrics_all_in_host() {
	int const n_nodes = DAG->len;
	const int metrics_len = m_N_padded; //necessario usare il round alla prossima potenza del due perché altrimenti il sort non potrebbe funzionare
	queue = entrypoints;
	...
	print(metrics, metrics_len, " ", true);
...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		...
		cin >> dataSetName;
		cout << "Vettorizzare? s/n: ";
		cin >> userResponseToVectorizeQuestion;
	} else {
		dataSetName = argv[1];
		if(argc > 2)isVector4Version = (strcmp(userResponseToVectorizeQuestion.c_str(), "s") == 0);
	}

	start_time = std::chrono::system_clock::now();

	if(isVector4Version)ocl_init("./kernels/graph_init.ocl","entry_discover", "compute_metrics_4");
	else ocl_init("./kernels/graph_init.ocl","entry_discover", "compute_metrics");
	...
	DAG = initDagWithDataSet(dataSetName);
	...
	if(isVector4Version)compute_metrics_evt = compute_metrics_all_in_host();
	else compute_metrics_evt = compute_metrics();
...
void ocl_init(const char* progName, const char* kernelNameEntryDiscover, const char *kernelNameComputeMetrics) {
	...
	cl_program prog2 = create_program("./kernels/sort.ocl", ctx, d);
